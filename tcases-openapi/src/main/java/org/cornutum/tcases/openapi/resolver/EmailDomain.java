...
public class EmailDomain extends AbstractStringDomain
  ...
  protected String newValue( Random random, int length)
    ...
    int localLength = Math.max( random.nextInt( localLengthMax) + 1, localLengthMin);
    int domainLength = partsLength - localLength;

    return
      new StringBuilder()
      .append( randomPathOf( random, localPartChars_, localLength, localLength))
      .append( "@")
      .append( randomPathOf( random, domainPartChars_, domainLength, MAX_LABEL))
      .append( ".")
      .append( topLevels_[ random.nextInt( topLevels_.length)])
      .toString();
    }
   ...
   */
  private static String randomPathOf( Random random, String chars, int length, int maxPartLength)
    {
    ...
    return
      ...
      .mapToObj( i -> randomStringOf( random, chars, i==0? prefixLength : partLength))
      .collect( joining( "."));
    }
   ...
   */
  private static String randomStringOf( Random random, String chars, int length)
    {
